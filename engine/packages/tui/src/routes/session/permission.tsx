          ...
          body={
            <Switch>
              <Match when={props.request.always.length === 1 && props.request.always[0] === "*"}>
                <TextBody title={"This will allow " + props.request.permission + " until Grepleaks is restarted."} />
              </Match>
              <Match when={true}>
                <box paddingLeft={1} gap={1}>
                  <text fg={theme.textMuted}>This will allow the following patterns until Grepleaks is restarted</text>
                  <box>
      ...
      <Match when={store.stage === "permission"}>
        {(() => {
          const info = () => {
            ...
            if (permission === "host_run") {
              const meta = props.request.metadata ?? {}
              const argv = Array.isArray(meta.argv) ? meta.argv.filter((arg): arg is string => typeof arg === "string") : []
              return {
                icon: "→",
                title: `Run on your host (${typeof meta.os === "string" ? meta.os : "outside Docker"})`,
                body: (
                  <box paddingLeft={1} gap={1}>
                    <text fg={theme.text}>{argv.map((arg) => JSON.stringify(arg)).join(" ")}</text>
                    <text fg={theme.textMuted}>{"Directory: " + (typeof meta.cwd === "string" ? meta.cwd : "host workspace")}</text>
                    <text fg={theme.textMuted}>This command runs directly on your machine with your user permissions.</text>
                  </box>
                ),
              }
            }
  ...
  return (
    ...
    >
      <box gap={1} paddingLeft={1} paddingRight={3} paddingTop={1} paddingBottom={1}>
        ...
        <box paddingLeft={1}>
          <text fg={theme.textMuted}>Tell Grepleaks what to do differently</text>
        </box>
      </box>
      <box
